...
<head>
    ...
    <style>
        :root { color-scheme: light dark; --bg:#f7f8fa; --card:#fff; --text:#202b39; --muted:#526174; --line:#dde3ea; --accent:#0f766e; --soft:#f0fdfa; font-family:system-ui,sans-serif; color:var(--text); background:var(--bg); }
        * { box-sizing:border-box; } body { margin:0; } a { color:var(--accent); text-underline-offset:4px; } a:focus-visible, input:focus-visible, button:focus-visible { outline:3px solid var(--accent); outline-offset:4px; }
        ...
        .hero { background:var(--soft); border:1px solid var(--line); border-radius:24px; padding:40px; margin:12px 0 30px; } .eyebrow { font-size:12px; text-transform:uppercase; letter-spacing:.15em; font-weight:750; color:var(--accent); } h1 { font-size:clamp(30px,5vw,48px); line-height:1.15; letter-spacing:-.035em; margin:14px 0; max-width:700px; } p { line-height:1.75; } .hero p { max-width:680px; color:var(--muted); } .search { margin-top:24px; max-width:660px; } label { display:block; font-weight:650; margin-bottom:8px; } .search-row { display:flex; gap:10px; } input { min-width:0; width:100%; padding:14px; border:1px solid var(--line); border-radius:10px; font:inherit; background:var(--card); color:var(--text); } .primary { background:#0f766e; color:#fff; border-color:#0f766e; }
        ...
        @media(prefers-color-scheme:dark) { :root { --bg:#141b24; --card:#1d2733; --text:#edf0f5; --muted:#b6c2d1; --line:#354254; --accent:#5eead4; --soft:#123b3a; } }
        article:target { border-color:var(--accent); box-shadow:0 0 0 2px var(--accent); }
        aside a[aria-current="location"] { background:var(--soft); color:var(--accent); font-weight:750; border-left:3px solid var(--accent); }
